Game.Canvas.Scene.New({
	...
	materials: {
		images: {
			background: "Graphics/Pictures/transparent.png",
	...
	drawText: function(_text, sprite_b, x, y, style) {
		style = style || {};
		var text = Game.Canvas.Text.new(this, _text);
		text.style({
	...
	displayActors: function(stage, onSelect) {
		...
		for (var i = 0; i < 4; i++) {
			...
			var img = Game.Canvas.Materials.get("characters_" + actor.graphic),
				margin = 20;
	...
	__index: function(stage) {
		...
		this._esc(function() {
			Game.Canvas.Scene.exit("Scene_Menu");
		});
	...
	_transition: function(type, elements, callback) {
		...
		for (var i = 0; i < elements.length; i++) {
			el = elements[i];
			Game.Canvas.Timeline.New(el.el).to(el.to, 40, easing).call(finish);
		}
	...
	*/
	...
	__items: function(stage, skills) {
		...
		var scroll = Game.Canvas.Scrolling.new(this);
		...
		var choice_actor = this.createElement(),
			actors = this.displayActors(stage, function(id) {
				...
				Game.Canvas.Timeline.New(choice_actor).to({
					x: _canvas.width
				}, 40).call();
	...
	__status: function(stage, actor_id) {
		...
		var pts = data_actor.getAllParamsPoint(),
			pos = 1;
		...
		var params = data_actor.getCurrentParam(),
			pos2 = 1;
		...
		this.transitionStart([{
			el: battler,
			to: {
				x: _canvas.width - battler.img.width
			}
		}, {
			el: body,
			to: {
	...
	createCursor: function(stage, width, height, btns, callback) {
		callback = callback || {};
		var box = Game.Canvas.Window.new(this, width, height);
	...
	exit: function() {
		Game.Canvas.Scene.get("Scene_Map").pause(false);
		Game.Canvas.Scene.get("Scene_Map").keysAssign();
	}
});