Game.Canvas.Scene.New({
	...
	displayActors: function(stage, onSelect) {
		...
		for (var i=0 ; i < 4 ; i++) {
			...
			var img = Game.Canvas.Materials.get("characters_" + actor.graphic), margin = 20;
	...
	__items: function(stage, skills) {
		...
		var choice_actor = this.createElement(),
			actors = this.displayActors(stage, function(id) {
				...
				Game.Canvas.Timeline.New(choice_actor).to({x: _canvas.width}, 40).call();
		...
		box.cursor.select(function(el) {
			...
			Game.Canvas.Timeline.New(choice_actor).to({x: _canvas.width - actors.width}, 40).call();
		});
